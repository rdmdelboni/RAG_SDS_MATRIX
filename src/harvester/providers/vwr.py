...
class VWRProvider(BaseSDSProvider):
    """SDS Provider for VWR/Avantor."""

    BASE_URL = "https://us.vwr.com"
    SEARCH_URL = "https://us.vwr.com/store/search/searchMSDS.jsp"
    ...
    def search(self, cas_number: str) -> List[SDSMetadata]:
        ...
        try:
            ...
            logger.debug("VWR search failed for %s: %s", cas_number, exc)
            return []
    ...
    def download(self, url: str, destination: Path) -> bool:
        try:
            ...
            logger.debug("VWR download failed for %s: %s", url, exc)
